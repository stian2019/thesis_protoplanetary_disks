 ...
 *********************************************************************** */
{
        ...
        //cs_a     = (0.05*grid->x[IDIR][i]*d_global->Vc[VX2][k][j][i]/grid->x[IDIR][i])^2;
        ...
        if (g_dir == IDIR) {
                ...
                for (i = beg; i <= end; i++) {
                        p->a2[i] = pow(0.05*grid->x[IDIR][i]*d_global->Vc[VX2][k][j][i]/grid->x[IDIR][i],2.);
                        //("%f,", p->a2[i]*d_global->Vc[RHO][k][j][i]);
                }

        }else if (g_dir == JDIR) {

                x2 = (pos == FACE_CENTER ? grid->xr[JDIR] : grid->x[JDIR]);
                for (j = beg; j <= end; j++) p->a2[j] = pow(0.05*grid->x[IDIR][i]*d_global->Vc[VX2][k][j][i]/grid->x[IDIR][i],2.);

        }else if (g_dir == KDIR) {

                x3 = (pos == FACE_CENTER ? grid->xr[KDIR] : grid->x[KDIR]);
                for (k = beg; k <= end; k++) p->a2[k] = pow(0.05*grid->x[IDIR][i]*d_global->Vc[VX2][k][j][i]/grid->x[IDIR][i],2.);
